(** QCheck-STM State Machine Test for LockFreeStack (builtin list version) *)
...
module Spec = struct
  ...
  let next_state c state =
    match c with
    | Push i -> { contents = i :: state.contents }
    | Try_pop | Pop ->
  ...
  let run c d =
    match c with
    | Push i -> Res (unit, LFS.push d i)
    | Try_pop -> Res (option int, LFS.try_pop d)
    | Pop -> Res (result int exn, protect LFS.pop d)
...
let run_test test_name =
  ...
  | "sequential" | "seq" ->
      ...
      let seq_test = Seq.agree_test ~count:5000
        ~name:"LockFreeStack (builtin list) sequential" in
      QCheck_base_runner.run_tests ~verbose:true [seq_test]
  | "concurrent" | "conc" ->
      Printf.printf "Running concurrent test (should pass)...\n\n%!";
      let arb_cmds_par =
        Dom.arb_triple 12 8 Spec.arb_cmd Spec.arb_cmd Spec.arb_cmd in
      let conc_test =
        QCheck.Test.make ~retries:10 ~count:200
          ~name:"LockFreeStack (builtin list) concurrent" arb_cmds_par
        @@ fun triple ->
        QCheck.assume (Dom.all_interleavings_ok triple);
        repeat 12 Dom.agree_prop_par triple in
      QCheck_base_runner.run_tests ~verbose:true [conc_test]
  | "all" ->
      Printf.printf "Running all tests...\n\n%!";
      let tests = [
        Seq.agree_test ~count:5000
          ~name:"LockFreeStack (builtin list) sequential";
        QCheck.Test.make ~retries:10 ~count:500
          ~name:"LockFreeStack (builtin list) concurrent"
...
let () =
  let test_name =
    if Array.length Sys.argv > 1 then Sys.argv.(1) else "all" in
  Printf.printf "QCheck-STM Tests for LockFreeStack (builtin list)\n\n";
