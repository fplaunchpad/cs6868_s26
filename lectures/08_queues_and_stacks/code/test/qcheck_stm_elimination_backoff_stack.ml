(** QCheck-STM State Machine Test for EliminationBackoffStack

    This test uses QCheck-STM to verify the elimination backoff
    stack's correctness against a sequential specification (a simple
    list-based unbounded LIFO).
...
module Spec = struct
  ...
  let run c d =
    match c with
    | Push i ->
        Res (unit, EBS.push d i)
    | Try_pop ->
        Res (option int, EBS.try_pop d)
    | Pop ->
        Res (result int exn, protect EBS.pop d)
...
let run_test test_name =
  ...
  | "sequential" | "seq" ->
      ...
      let seq_test = Seq.agree_test ~count:5000 ~name:"EliminationBackoffStack sequential" in
  ...
  | "concurrent" | "conc" ->
      ...
      in
      let conc_test =
        QCheck.Test.make ~retries:10 ~count:200 ~name:"EliminationBackoffStack concurrent" arb_cmds_par
        @@ fun triple ->
  ...
  | "all" ->
      ...
      let tests = [
        Seq.agree_test ~count:5000 ~name:"EliminationBackoffStack sequential";
        QCheck.Test.make ~retries:10 ~count:500 ~name:"EliminationBackoffStack concurrent"
          (Dom.arb_triple 12 8 Spec.arb_cmd Spec.arb_cmd Spec.arb_cmd)
...
let () =
  let test_name =
    if Array.length Sys.argv > 1 then
      ...
      "all"
  in
  Printf.printf "QCheck-STM Tests for EliminationBackoffStack\n\n";
