(** QCheck-STM State Machine Test for BoundedQueue

    This test uses QCheck-STM to verify the bounded queue's correctness
    against a sequential specification (a simple list-based FIFO model
    with bounded capacity).

    We use [try_enq] and [try_deq] (non-blocking) to avoid deadlocks
    in the test harness.
...
module Spec = struct
  ...
  (** Model state: a bounded FIFO queue as a list (head = front) *)
  type state = {
    contents : int list;
    capacity : int;
  }
  ...
  let next_state c state =
    ...
    | Try_enq i ->
        if List.length state.contents < state.capacity then
          ...
          state  (* Full, no change *)
    | Try_deq ->
        (match state.contents with
         | [] -> state  (* Empty, no change *)
  ...
  let postcond c state res =
    ...
    | Try_enq _, Res ((Bool, _), actual) ->
        (* try_enq returns true if there was space *)
...
let run_test test_name =
  ...
  | "sequential" | "seq" ->
      ...
      let seq_test = Seq.agree_test ~count:5000 ~name:"BoundedQueue sequential" in
  ...
  | "concurrent" | "conc" ->
      ...
      in
      let conc_test =
        QCheck.Test.make ~retries:10 ~count:200 ~name:"BoundedQueue concurrent" arb_cmds_par
        @@ fun triple ->
  ...
  | "all" ->
      ...
      let tests = [
        Seq.agree_test ~count:5000 ~name:"BoundedQueue sequential";
        QCheck.Test.make ~retries:10 ~count:500 ~name:"BoundedQueue concurrent"
          (Dom.arb_triple 12 8 Spec.arb_cmd Spec.arb_cmd Spec.arb_cmd)
...
let () =
  let test_name =
    if Array.length Sys.argv > 1 then
      ...
      "all"
  in
  Printf.printf "QCheck-STM Tests for BoundedQueue\n\n";
