...
val create : unit -> t
(** Create a new trigger in the Initial state. *)

val signal : t -> bool
(** Signal the trigger. If the trigger has already been signaled, returns
    [false].  Otherwise, returns [true]. If a callback was registered via
    [on_signal], it is invoked. *)

val on_signal : t -> (unit -> unit) -> bool
(** [on_signal t cb] registers [cb] to be called when [t] is signaled.
    Returns [true] if the callback was registered (trigger was Initial),
